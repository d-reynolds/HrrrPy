...
projection_Lon = -97.5
projection_Lat = 38.5
epsg_dest = 'epsg:26911'
##-----------------------------------------------


## Function to take netCDF file and write data to met forcing file for SnowModel

#Ensure masked values are filled in with SnowModel default of -9999.0
...
#Prepare lat/lons for conversion
...
proj_gdf = met_gdf.to_crs({'init':epsg_dest})#,str(epsg_code))})
...
#Convert wind components to speed and direction, with direction convention of direction that wind
#is blowing from
...
#Mask all variables to valid nodes
...
#Print out file according to SnowModel input file convention
